...
class AuthFeature extends AppFeature {
  ...
  Future<void> initialize() async {
    ...
    // Expose the use cases to the custom Stac auth actions (core.login, ...).
    ...
    _splashBloc = SplashBloc(
      // Pure read: the route guard marks `showOnce` screens seen when it lets
      // them through (i.e. when actually shown) — see [createRouter].
      resolveStart: () => AppFlow.instance.resolveStart(
        hasSession: CacheManager.hasSession,
        seen: CacheManager.seen,
      ),
    );
