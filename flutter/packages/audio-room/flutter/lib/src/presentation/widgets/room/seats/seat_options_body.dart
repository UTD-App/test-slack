...
import '../invite/invite_to_mic_sheet.dart';
    ...
    return Padding(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          Text(
            context.trArgs(AudioRoomKeys.seat, {'index': '${seatIndex + 1}'}),
          ...
          if (isSeatEmpty) ...[
            ...
            if (isAdmin)
              SeatOptionTile(
                ...
                label: isLocked ? context.tr(AudioRoomKeys.unlockSeat) : context.tr(AudioRoomKeys.lockSeat),
                onTap: () async {
            ...
            if (!isLocked || isAdmin)
              SeatOptionTile(
                ...
                label: controller.localSeatIndex >= 0
                    ? context.tr(AudioRoomKeys.switchSeat)
                    : context.tr(AudioRoomKeys.takeSeat),
          ...
          if (isOwnSeat) ...[
            ...
            SeatOptionTile(
              icon: Icons.person,
              label: context.tr(AudioRoomKeys.myProfile),
              onTap: () {
                Navigator.of(context).pop();
  ...
  Future<void> _takeSeat(BuildContext context) async {
    ...
    if (!ok && context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(context.tr(AudioRoomKeys.couldNotTakeSeat))),
      );
    }
  }
}
